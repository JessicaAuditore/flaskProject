...
class MobileNetV3(nn.Module):
    def __init__(self, num_classes=1000, type='large'):
        ...
        if type == 'large':
            ...
            # 输入为1/8，输出为1/16
            self.large_bottleneck3 = nn.Sequential(
        ...
        # self.classifier = nn.Linear(in_features=128000, out_features=num_classes)
    ...
    def forward(self, x):
        ...
        if self.type == 'large':
            x0 = self.large_bottleneck0(x)
            x1 = self.large_bottleneck1(x0)  # 输出为1/4
            x2 = self.large_bottleneck2(x1)  # 输出为1/8
            x3 = self.large_bottleneck3(x2)  # 输出为1/16
            x4 = self.large_bottleneck4(x3)  # 输出为1/32

            x5 = self.large_last_stage(x4)  # 输出为1/32
